    ...
    if (pszAddress != NULL && nSize > 0)
    {
        ...
        if (GetLocalName(szHost, HOSTNAME_SIZE))
        {
...
#ifdef _UNICODE
            char pstrHost[HOSTNAME_SIZE] = { 0 };
            uni2ansi(CP_ACP,szHost, pstrHost);
#else
            ...
            if (inet_ntop(addr_store.ss_family, addr, szAddress, MAX_PATH) != NULL)
            {
                // Unicode conversion
#ifdef _UNICODE
                return (0 != ansi2uni(CP_ACP,szAddress, pszAddress));
#else
...
bool CSocketHandle::GetMacAddress(LPCSTR sIP,LPSTR szMac,UINT nSize)  
{  
	...
	while ( pinfo != NULL )
	{
		...
		{
			::StringCbCopyA(szMyIp,MAX_PATH-1,pAddressList->IpAddress.String);
			if ( !stricmp(szMyIp,sIP))
			{
				::StringCchPrintfA(szMac,nSize,"%02X:%02X:%02X:%02X:%02X:%02X",pinfo->Address[0],pinfo->Address[1],pinfo->Address[2],
